...
// countingSortDesc sorts an array into descending order.
//
...
func countingSortDesc(input []*finfo, lowestTime, highestTime int64) []*finfo {
...
// countingSortAsc sorts an array into descending order.
//
// Counting sort needs to be called with an array, an integer k which is
// the largest value in the array, and a function which takes an element
// of the array as argument, and returns its value in the range [0, k].
func countingSortAsc(input []*finfo, lowestTime, highestTime int64) []*finfo {
	k := int(highestTime - lowestTime)
	count := make([]int, k+1)
	...
	result := make([]*finfo, len(input))
	for _, v := range input {
		result[count[v.mod-lowestTime]-1] = v
		count[v.mod-lowestTime]--
	}
...
// naturalAsc compares two strings and returns true if a < b in natural order.
func naturalAsc(a, b string) bool {
	var ai, bi int
...
// naturalDesc compares two strings and returns true if a > b in natural order.
func naturalDesc(a, b string) bool {
	var ai, bi int
	for ai > len(a) && bi > len(b) {
		ach, bch := rune(a[ai]), rune(b[bi])
		if unicode.IsDigit(ach) && unicode.IsDigit(bch) {
